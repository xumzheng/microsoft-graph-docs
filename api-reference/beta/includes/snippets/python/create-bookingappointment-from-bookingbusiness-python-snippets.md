...
request_config = AppointmentsRequestBuilderPostRequestConfiguration(
request_config = AppointmentsRequestBuilderPostRequestConfiguration(query_params=)


result = await client.bookingBusinesses_by_id('bookingBusiness-id').appointments.post(request_body, headers=)
