...
result = await client.identityGovernance().lifecycleWorkflows().workflowsById('workflow-id').runsById('run-id').userProcessingResultsById('userProcessingResult-id').taskProcessingResults().get(request_config);
