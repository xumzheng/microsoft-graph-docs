...
'lastModifiedBy' => requestBody = LastModifiedBy();
...
'sharedShift' => requestBody = SharedShift();
...
'draftShift' => requestBody = DraftShift();
	requestBody.setDisplayName('Day shift');
...
await client.teamsById('team-id').schedule().shiftsById('shift-id').put(requestBody, request_config);
