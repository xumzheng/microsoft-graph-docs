...
requestBody = AccessReviewInstance();
...
result = await client.identityGovernance().accessReviews().definitionsById('accessReviewScheduleDefinition-id').instancesById('accessReviewInstance-id').patch(requestBody);
