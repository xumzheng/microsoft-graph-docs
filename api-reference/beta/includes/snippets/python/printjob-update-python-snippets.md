...
configurationmargin = Print_margin()
...
result = await client.print.printers.by_printer_id('printer-id').jobs.by_job_id('printJob-id').patch(request_body = request_body)
