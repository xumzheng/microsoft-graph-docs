...
requestBody = BookingStaffMemberBase();
...
result = await client.solutions().bookingBusinessesById('bookingBusiness-id').staffMembersById('bookingStaffMemberBase-id').patch(requestBody);
