...
requestBody = CreateNewVersionPostRequestBody();
workflow = Workflow();
...
result = await client.identityGovernance().lifecycleWorkflows().workflowsById('workflow-id').identityGovernanceCreateNewVersion().post(requestBody);
