...
request_config = EvaluateRemovalRequestBuilderPostRequestConfiguration(query_params=)
headers['User-Agent'] = "ContosoLOBApp/1.0"


result = await client.users_by_id('user-id').security.informationProtection.sensitivityLabels.securityevaluateRemoval.post(request_body, request_config, headers=request_config)
