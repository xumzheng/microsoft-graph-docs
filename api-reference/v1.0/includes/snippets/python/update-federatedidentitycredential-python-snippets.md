...
request_configuration = FederatedIdentityCredentialRequestBuilderPatchRequestConfiguration(
)


result = await client.applications_by_id('application-id').federatedIdentityCredentials_by_id('federatedIdentityCredential-id').patch(request_body = request_body)
