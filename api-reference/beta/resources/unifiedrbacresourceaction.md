...
|authenticationContextId|String|Conditional Access authentication context id that is assigned to the permission to add a protected action, such as `c1`. This property is only shown when requested using `$select`.|
...
|isAuthenticationContextSettable|Boolean|This property indicates is the current resource can be assigned a Conditional Access authentication context .This property is only shown when requested using `$select`.|
