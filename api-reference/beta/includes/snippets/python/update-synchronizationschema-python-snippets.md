...
requestBody = SchemaPutRequestBody();
additionalData = [
...
await client.servicePrincipalsById('servicePrincipal-id').synchronization().jobsById('synchronizationJob-id').schema().put(requestBody);
