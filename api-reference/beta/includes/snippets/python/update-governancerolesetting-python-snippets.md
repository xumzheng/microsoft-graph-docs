...
request_config = GovernanceRoleSettingRequestBuilderPatchRequestConfiguration(
request_config = GovernanceRoleSettingRequestBuilderPatchRequestConfiguration(query_params=)


result = await client.privilegedAccess_by_id('privilegedAccess-id').roleSettings_by_id('governanceRoleSetting-id').patch(request_body, headers=)
