...
requestBody = EvaluateRemovalPostRequestBody();
...
result = await client.usersById('user-id').security().informationProtection().sensitivityLabels().securityEvaluateRemoval().post(requestBody, request_config);
