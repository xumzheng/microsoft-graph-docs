...
A delta query in Microsoft Graph enables applications to discover newly created, updated, or deleted entities without performing a full read of the target resource with every request. Applications calling Microsoft Graph can use the delta query to efficiently synchronize changes with a local data store.

The delta query in Microsoft Graph is inferred when the `delta` function is appended to a URL path. For example, `https://graph.microsoft.com/v1.0/users/delta`. `/delta` is a shortcut for the fully qualified name `/microsoft.graph.delta` that you see in requests generated by the Microsoft Graph SDKs.
...
### Adding $select to a $deltaLink isn't supported
