...
requestBody = UpdateAudienceByIdPostRequestBody();
...
await client.admin().windows().updates().deploymentsById('deployment-id').audience().windowsUpdatesUpdateAudienceById().post(requestBody);
