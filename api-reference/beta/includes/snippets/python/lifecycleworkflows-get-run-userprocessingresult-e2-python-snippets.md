...
result = await client.identityGovernance.lifecycleWorkflows.workflows_by_id('workflow-id').runs_by_id('run-id').userProcessingResults_by_id('userProcessingResult-id').get(request_configuration = request_configuration)
