...
result = await client.solutions().businessScenariosById('businessScenario-id').planner().planConfiguration().localizationsById('plannerPlanConfigurationLocalization-id').patch(requestBody);
