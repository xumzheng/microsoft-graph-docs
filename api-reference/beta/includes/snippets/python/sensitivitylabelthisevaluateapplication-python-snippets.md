...
requestBody = EvaluateApplicationPostRequestBody();
...
result = await client.usersById('user-id').security().informationProtection().sensitivityLabels().securityEvaluateApplication().post(requestBody, request_config);
