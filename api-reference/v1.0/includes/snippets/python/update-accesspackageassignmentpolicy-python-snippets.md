...
requestBody.setAllowedTargetScope(AllowedTargetScope('alldirectoryusers'));
...
result = await client.identityGovernance().entitlementManagement().assignmentPoliciesById('accessPackageAssignmentPolicy-id').put(requestBody);
