...
requestBody = $valuePutRequestBody();
additionalData = [
...
await client.identity().b2cUserFlowsById('b2cIdentityUserFlow-id').languagesById('userFlowLanguageConfiguration-id').overridesPagesById('userFlowLanguagePage-id').value().put(requestBody);
