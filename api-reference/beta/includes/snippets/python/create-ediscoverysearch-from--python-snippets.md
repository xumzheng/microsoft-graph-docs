...
request_body.setDisplayName('My search 2')

request_body.setDescription('My first search')
...
request_config = SearchesRequestBuilderPostRequestConfiguration(
request_config = SearchesRequestBuilderPostRequestConfiguration(query_params=)


result = await client.security.cases.ediscoveryCases_by_id('ediscoveryCase-id').searches.post(request_body, headers=)
