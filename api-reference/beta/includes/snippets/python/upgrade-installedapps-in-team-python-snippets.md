...
request_config = UpgradeRequestBuilderPostRequestConfiguration(
request_config = UpgradeRequestBuilderPostRequestConfiguration(query_params=)


await client.teams_by_id('team-id').installedApps_by_id('teamsAppInstallation-id').upgrade.post(request_body, headers=)
