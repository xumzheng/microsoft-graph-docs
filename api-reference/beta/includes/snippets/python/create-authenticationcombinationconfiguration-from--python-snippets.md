...
result = await client.identity().conditionalAccess().authenticationStrengths().policiesById('authenticationStrengthPolicy-id').combinationConfigurations().post(requestBody);
