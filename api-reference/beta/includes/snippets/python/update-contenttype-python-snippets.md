...
request_config = ContentTypeRequestBuilderPatchRequestConfiguration(
request_config = ContentTypeRequestBuilderPatchRequestConfiguration(query_params=)


result = await client.sites_by_id('site-id').contentTypes_by_id('contentType-id').patch(request_body, headers=)
