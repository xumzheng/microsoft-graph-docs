...
requestBody = PlannerTaskConfiguration();
...
result = await client.solutions().businessScenariosById('businessScenario-id').planner().taskConfiguration().patch(requestBody);
