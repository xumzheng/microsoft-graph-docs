...
requestBody = StartHoldMusicPostRequestBody();
customPrompt = Prompt();
...
result = await client.communications().callsById('call-id').participantsById('participant-id').startHoldMusic().post(requestBody);
