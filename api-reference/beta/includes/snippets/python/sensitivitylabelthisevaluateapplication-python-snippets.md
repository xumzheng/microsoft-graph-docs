...
labelingOptionsDowngradeJustification = DowngradeJustification()
...
request_body.setLabelingOptions($labelingOptions)

request_config = EvaluateApplicationRequestBuilderPostRequestConfiguration(
request_config = EvaluateApplicationRequestBuilderPostRequestConfiguration(query_params=)
headers['User-Agent'] = "ContosoLobApp/1.0"


result = await client.users_by_id('user-id').security.informationProtection.sensitivityLabels.securityevaluateApplication.post(request_body, request_config, headers=request_config)
