...
request_body.setLoop(false)


request_config = PlayPromptRequestBuilderPostRequestConfiguration(
request_config = PlayPromptRequestBuilderPostRequestConfiguration(query_params=)


result = await client.communications.calls_by_id('call-id').playPrompt.post(request_body, headers=)
