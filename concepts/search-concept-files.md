...
You can specify the fields you want back in the response, as part of the **fields** sub-property in listItem or **listItem** sub-property in driveItem of a [searchHit](/graph/api/resources/searchhit) object in the response. This is a way to either trim down the response over the wire, or to request some specific properties that are not part of the out-of-the-box schema.

Note that property selection for custom properties in SharePoint is only available for **listItem**  and **driveItem** since these are SharePoint entities in Microsoft Graph that supports custom properties.
